...
namespace PowerPlant.WebApi.Client
{
    internal class InspectionsHandler
    {
        ...
        private readonly InspectionsWebApiClient _inspectionsWebApiClient;
        ...
        public InspectionsHandler()
        {
            ...
            _inspectionsWebApiClient = new InspectionsWebApiClient();
        }

        public async void CreateNewInspectionTicet(Member loggedMember)
        {
            ...
            while (!_inspectionsWebApiClient.ItemExistsAsync(itemName).Result)
            {
                _consoleManager.Clear();
            ...
            if (_inspectionsWebApiClient.OpenTicketExistsAsync(itemName).Result)
            {
                _consoleManager.Clear();
            ...
            var result = await _inspectionsWebApiClient.CreateAsync(new InspectionTicket
            {
        ...
        {
            _consoleManager.Clear();
            var tickets = _inspectionsWebApiClient.GetAllAsync().Result;
            ...
            foreach (var ticket in tickets)
            {
                ...
                _consoleManager.WriteLine($"Creation date:\t\t{ticket.CreationDate}");
                _consoleManager.WriteLine($"Assignment date:\t{ticket.AssignmentDate}");
                _consoleManager.WriteLine($"Termination date:\t{ticket.TerminationDate}");
        ...
        public async void AssignInspectionTicketAsync(Member loggedMember)
        {
            ...
            var result = await _inspectionsWebApiClient.AssignAsync(pickedTicket);
